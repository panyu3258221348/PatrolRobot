...
typedef struct
{
   ...
   float speed_factor;     // 无需配置，计算速度时使用的速度因子，设置时自动计算，speed_factor计算方式如下
   ...
   float motor_speed;       // 无需配置，当前电机速度mm/s，计算时使用
    float last_encoder_tick; // 无需配置，上次电机的编码器读数
...
public:
    ...
    void set_motor_param(uint8_t id);
    /**
     * @brief 设置运动学相关参数
     * 
     ...
     * @param angle_speed 
     */
    void kinematic_forward(float wheel1_speed, float wheel2_speed,float wheel3_speed, float wheel4_speed, float &line_speed, float &angle_speed);
     ...
     * @param motor_tick2 
     */
    void update_motor_ticks(uint64_t current_time, float speed1,float speed2,float speed3,float speed4);
     ...
     * @param dt 间隔时间dt
     */
    void update_bot_odom(uint32_t dt,float speed1,float speed2,float speed3,float speed4);
